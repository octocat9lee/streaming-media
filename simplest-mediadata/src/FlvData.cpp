...
uint reverse_bytes(byte *p, uint8_t c)
{
    int r = 0;
    int i;
    for(i = 0; i < c; i++)
...
int simplest_flv_parser(const std::string &url)
{
    ...
    FILE *flvFile = fopen(url.c_str(), "rb");
    if(flvFile == nullptr)
    {
    ...
    //FLV file header
    FLV_HEADER flv;
    fread((char *)&flv, 1, sizeof(FLV_HEADER), flvFile);
    printf("============== FLV Header ==============\n");
    ...
    fseek(flvFile, reverse_bytes((byte *)&flv.DataOffset, sizeof(flv.DataOffset)), SEEK_SET);
    ...
    do
    {
        uint previoustagsize = _getw(flvFile);

        TAG_HEADER tagheader;
        fread((void *)&tagheader, sizeof(TAG_HEADER), 1, flvFile);
        ...
        switch(tagheader.TagType)
        {
            case TAG_TYPE_AUDIO:
            {
                ...
                strcat(audiotag_str, "| ");
                char tagdata_first_byte = fgetc(flvFile);
                ...
                printf("%s", audiotag_str);

                //if the output file hasn't been opened, open it.
                if(output_a != 0 && audioFile == nullptr)
                {
                    audioFile = fopen("output.mp3", "wb");
                }
            ...
            case TAG_TYPE_VIDEO:
            {
                ...
                printf("%s", videotag_str);

                fseek(flvFile, -1, SEEK_CUR);

                //if the output file hasn't been opened, open it.
                if(videoFile == nullptr && output_v != 0)
                {
                    uint  previoustagsize_z = 0;
                    //write the flv header (reuse the original file's hdr) and first previoustagsize
                    videoFile = fopen("output.flv", "wb");
                    fwrite((char *)&flv, 1, sizeof(flv), videoFile);
                    fwrite((char *)&previoustagsize_z, 1, sizeof(previoustagsize_z), videoFile);
                }
                ...
                {
                    //TagHeader
                    fwrite((char *)&tagheader, 1, sizeof(tagheader), videoFile);
                    //TagData
                    for(int i = 0; i < data_size; i++)
                    {
                        fputc(fgetc(flvFile), videoFile);
                    }
                }
                else
                {
                ...
                //rewind 4 bytes, because we need to read the previous tag size again for the loop's sake
            ...
            default:
            {
                ...
                fseek(flvFile, reverse_bytes((byte *)&tagheader.DataSize, sizeof(tagheader.DataSize)), SEEK_CUR);
            }
        }
        printf("\n");
    } while(!feof(flvFile));
